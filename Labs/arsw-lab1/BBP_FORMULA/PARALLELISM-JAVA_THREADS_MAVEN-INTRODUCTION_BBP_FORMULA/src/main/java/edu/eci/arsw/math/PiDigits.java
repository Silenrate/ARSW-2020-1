    ...
    public static byte[] getDigits(int start, int count, int numThreads) throws InterruptedException {
        ...
        for (int i=0;i<numThreads;i++){
            ...
            if(i!=numThreads-1){
                ...
                threadCount=amount+(count%numThreads);
            }
            threads[i] = new PiThread(start,threadCount);
            threads[i].start();
            start += amount;
        ...
        for(int i=0;i<numThreads;i++){
            threads[i].join();
            System.arraycopy(threads[i].getAnswer(), 0, digits, inicio,threads[i].getAnswer().length);
            inicio += amount;
        }
        return digits;
    }
